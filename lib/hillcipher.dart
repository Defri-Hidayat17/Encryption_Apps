...
class _HillCipherPageState extends State<HillCipherPage> {
  ...
  String _result = '';
  List<List<int>> _autoKeyMatrix = [];

  // ========================================
  // 🔑 Membuat matriks kunci otomatis (2x2)
  // ========================================
  List<List<int>> _generateAutoKeyMatrix(int size) {
    final random = Random();
    List<List<int>> matrix = List.generate(size, (_) => List.filled(size, 0));

    for (int i = 0; i < size; i++) {
      for (int j = 0; j < size; j++) {
        matrix[i][j] = random.nextInt(26); // 0-25 untuk huruf A-Z
      }
    }
  ...
  String hillEncrypt(String text, List<List<int>> keyMatrix) {
    ...
    for (int i = 0; i < text.length; i += n) {
      List<int> vector = [];
      for (int j = 0; j < n; j++) {
        vector.add(text.codeUnitAt(i + j) - 65);
      }
      ...
      for (int val in cipherVector) {
        encrypted.write(String.fromCharCode(val + 65));
      }
    }
  ...
  void _autoEncrypt() {
    ...
    final keyMatrix = _generateAutoKeyMatrix(2); // matriks 2x2
  ...
  Future<void> _sendToWhatsApp() async {
    ...
    final keyStr = _matrixToString(_autoKeyMatrix);
...
📜 *Petunjuk Dekripsi:*
Gunakan matriks kunci "$keyStr" untuk membuka pesan ini.
━━━━━━━━━━━━━━━
...
💀 ENCRYPTED MESSAGE - CYBER OPS TEAM (Hill Cipher)
                ...
                child: Column(
                  ...
                  children: [
                      ...
                      Row(
                        children: [
                          ...
                          ElevatedButton.icon(
                            onPressed: () {
                              final info =
                                  'Gunakan matriks kunci "${_matrixToString(_autoKeyMatrix)}" untuk mendekripsi pesan ini.';
                    ...
                    const SelectableText(
                      ...
                      'Teks diubah menjadi vektor huruf, lalu dikalikan dengan matriks kunci modulo 26. '
                      'Metode ini cocok untuk blok teks 2 atau 3 huruf sekaligus.',
